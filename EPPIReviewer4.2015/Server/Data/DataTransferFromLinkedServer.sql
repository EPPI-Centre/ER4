...
SELECT CONTACT_ID, CONTACT, USERID, [PASSWD], LAST_LOGIN, DATE_CREATED, EMAIL FROM [epi2].EPPI_WEB.DBO.[TB_CONTACT]

INSERT INTO REVIEWER.DBO.TB_REVIEW(OLD_REVIEW_ID, OLD_REVIEW_GROUP_ID, REVIEW_NAME, DATE_CREATED, FUNDER_ID, REVIEW_NUMBER)
SELECT REVIEW_ID, REVIEW_GROUP_ID, REVIEW, DATE_REVIEW_CREATED, FUNDER_ID, REVIEW_NUMBER FROM [epi2].EPPI_WEB.DBO.TB_REVIEW
...
SELECT CONTACT_ID, REVIEW_ID, 
	...
	FROM [epi2].EPPI_WEB.DBO.TB_REVIEW_CONTACT as src
go
...
from ( select 
			...
			END,
			*
		from [epi2].EPPI_WEB.dbo.tb_ITEM 
	) a
GO
...
SELECT (SELECT REVIEWER.DBO.TB_SET_TYPE.SET_TYPE_ID FROM REVIEWER.DBO.TB_SET_TYPE
		...
		GUIDELINE, GUIDELINE_ID FROM [epi2].EPPI_WEB.DBO.TB_GUIDELINE as src
go
...
SELECT (SELECT REVIEWER.DBO.TB_ATTRIBUTE.ATTRIBUTE_ID FROM REVIEWER.DBO.TB_ATTRIBUTE
		...
		ATTR_ORDER
		FROM [epi2].EPPI_WEB.DBO.TB_GUIDE_STRUCT as src WHERE src.SUP_ATTRIBUTE_ID IS NULL
...
SELECT (SELECT REVIEWER.DBO.TB_ITEM.ITEM_ID FROM REVIEWER.DBO.TB_ITEM
	...
	WHERE src.REVIEW_ID = 'REV327' OR src.REVIEW_ID = 'REV142' OR src.REVIEW_ID = 'REV203' 
go
...
SELECT DISTINCT(REVIEW_ID), 3, 'Inductive codes'  FROM [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE
...
SELECT DISTINCT (SELECT REVIEWER.DBO.TB_ITEM.ITEM_ID FROM REVIEWER.DBO.TB_ITEM
		WHERE REVIEWER.DBO.TB_ITEM.OLD_ITEM_ID = [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE_EXTRACT_ATTR.ITEM_ID COLLATE Latin1_General_CI_AS),
		...
		(SELECT REVIEWER.DBO.TB_ITEM_SET.ITEM_SET_ID FROM REVIEWER.DBO.TB_ITEM_SET WHERE
			REVIEWER.DBO.TB_ITEM_SET.ITEM_ID = 
				(SELECT REVIEWER.DBO.TB_ITEM.ITEM_ID FROM REVIEWER.DBO.TB_ITEM
					WHERE REVIEWER.DBO.TB_ITEM.OLD_ITEM_ID = [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE_EXTRACT_ATTR.ITEM_ID COLLATE Latin1_General_CI_AS
				AND REVIEWER.DBO.TB_ITEM_SET.SET_ID = (SELECT REVIEWER.DBO.TB_SET.SET_ID FROM REVIEWER.DBO.TB_SET
					WHERE REVIEWER.DBO.TB_SET.OLD_GUIDELINE_ID = 
					
					(SELECT REVIEW_ID FROM [epi2].EPPI_WEB.DBO.tb_INDUCTIVE_CODE
						WHERE [epi2].EPPI_WEB.DBO.tb_INDUCTIVE_CODE.INDUCTIVE_CODE_ID = [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE_EXTRACT_ATTR.INDUCTIVE_CODE_ID) COLLATE Latin1_General_CI_AS))),
					

		(SELECT REVIEWER.DBO.TB_ATTRIBUTE.ATTRIBUTE_ID FROM REVIEWER.DBO.TB_ATTRIBUTE
		WHERE REVIEWER.DBO.TB_ATTRIBUTE.OLD_ATTRIBUTE_ID =
			CAST('IC' + CAST([epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE_EXTRACT_ATTR.INDUCTIVE_CODE_ID AS NVARCHAR(6)) AS NVARCHAR(10)) COLLATE Latin1_General_CI_AS)
		
	FROM [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE_EXTRACT_ATTR
*/
...
INNER JOIN TB_ITEM_DOCUMENT ON
			TB_ITEM_DOCUMENT.OLD_EXTRACT_ATTR_IDENTITY =
				(SELECT [IDENTITY]
					FROM [epi2].EPPI_WEB.DBO.TB_EXTRACT_ATTR as src1 WHERE 
					src1.ATTRIBUTE_ID =src.ATTRIBUTE_ID
					AND src1.ITEM_ID = src.ITEM_ID
					AND src1.GUIDELINE_ID = src.GUIDELINE_ID)
...
FROM [epi2].EPPI_WEB.DBO.tb_REFERENCE_ELEMENT_GROUPS WHERE TYPE = 'EXCLUSION'
...
INNER JOIN [epi2].eppi_web.dbo.tb_REFERENCE_ITEM_FOR_SCREENING as src
	ON 'SCR' + CAST(src.ITEM_ID AS NVARCHAR(10)) = TB_ITEM.OLD_ITEM_ID COLLATE Latin1_General_CI_AS
INNER JOIN TB_REVIEW ON TB_REVIEW.OLD_REVIEW_ID = src.REVIEW_ID COLLATE Latin1_General_CI_AS
	AND src.REVIEW_ID = 'REV373'
