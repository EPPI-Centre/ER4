...
--set @revID = 'REV292' 
--set @revID = 'REV253'
--set @revID = 'REV241' --EPPI-Reviewer demonstration
--set @revID = 'REV285'
--set @revID = 'REV127'
--set @revID =  'REV236'
--set @revID =  'REV409'
--set @revID =  'REV142' --EPPI-Reviewer Developments - always throws errors!!!!!!!!!!!!
--set @revID =  'REV245' --EPPI-Reviewer training 1
set @revID =  'REV304' --EPPI-Reviewer training 2
--set @revID =  'REV154'
...
SELECT REVIEW_ID,
 ...
 REVIEW_NUMBER ,
 DATEADD(M,6, GETDATE())
FROM [epi2].EPPI_WEB.DBO.TB_REVIEW s where s.review_id = @revID
...
SELECT CONTACT_ID, REVIEW_ID, 
	...
	FROM [epi2].EPPI_WEB.DBO.TB_REVIEW_CONTACT as src where src.REVIEW_ID = @revID
...
from ( select 
			...
			END,
			*
		from [epi2].EPPI_WEB.dbo.tb_ITEM src
		where exists (SELECT REVIEW_ID from [epi2].EPPI_WEB.dbo.TB_ITEM i inner join
						[epi2].EPPI_WEB.dbo.tb_ITEM_REVIEW j
...
From tb_ITEM as T 
	inner join [epi2].EPPI_WEB.dbo.tb_ITEM_AUTHOR as S 
	on T.OLD_ITEM_ID COLLATE SQL_Latin1_General_CP1_CI_AS = S.ITEM_ID 
	inner join [epi2].EPPI_WEB.dbo.tb_ITEM_REVIEW j 
...
from ( select 
	...
	IMPORTED_REF_ID
		from [epi2].EPPI_WEB.dbo.tb_REFERENCE_ITEM_FOR_SCREENING src
...
SELECT (SELECT reviewer.DBO.TB_SET_TYPE.SET_TYPE_ID FROM reviewer.DBO.TB_SET_TYPE
		...
		rtrim(GUIDELINE), src.GUIDELINE_ID FROM [epi2].EPPI_WEB.DBO.TB_GUIDELINE as src
		inner join [epi2].EPPI_WEB.DBO.tb_REVIEW_GUIDELINE as rg on rg.GUIDELINE_ID = src.GUIDELINE_ID
WHERE not EXISTS 
...
SELECT ti.ITEM_ID,
	r.REVIEW_ID,
	'TRUE', NULL, 'FALSE'
	FROM [epi2].EPPI_WEB.DBO.TB_ITEM_REVIEW as src 
...
SELECT (SELECT reviewer.DBO.TB_CONTACT.CONTACT_ID FROM reviewer.DBO.TB_CONTACT
	...
	FROM [epi2].EPPI_WEB.DBO.TB_ATTRIBUTE as src
		inner join [epi2].EPPI_WEB.DBO.tb_GUIDE_STRUCT gs
		ON src.ATTRIBUTE_ID = gs.ATTRIBUTE_ID
		inner join [epi2].EPPI_WEB.DBO.tb_REVIEW_GUIDELINE rg 
...
SELECT (SELECT reviewer.DBO.TB_SET_TYPE.SET_TYPE_ID FROM reviewer.DBO.TB_SET_TYPE
		...
		rtrim(GUIDELINE), src.GUIDELINE_ID
		 FROM [epi2].EPPI_WEB.DBO.TB_GUIDELINE as src
		 inner join [epi2].EPPI_WEB.DBO.tb_REVIEW_GUIDELINE rg on 
...
SELECT (SELECT reviewer.DBO.TB_ATTRIBUTE.ATTRIBUTE_ID FROM reviewer.DBO.TB_ATTRIBUTE
		...
		ATTR_ORDER
		FROM [epi2].EPPI_WEB.DBO.TB_GUIDE_STRUCT as src 
		inner join [epi2].EPPI_WEB.DBO.tb_REVIEW_GUIDELINE rg on 
		 src.GUIDELINE_ID = rg.GUIDELINE_ID
...
SELECT (SELECT top 1 reviewer.DBO.TB_ITEM.ITEM_ID FROM reviewer.DBO.TB_ITEM
		...
		(SELECT reviewer.DBO.TB_CONTACT.CONTACT_ID FROM reviewer.DBO.TB_CONTACT
			WHERE reviewer.DBO.TB_CONTACT.OLD_CONTACT_ID = src.CONTACT_ID COLLATE Latin1_General_CI_AS)
		FROM [epi2].EPPI_WEB.DBO.TB_DATA_EXTRACTION_GUIDELINE as src
		inner join [epi2].EPPI_WEB.DBO.tb_REVIEW_GUIDELINE rg on 
		src.GUIDELINE_ID = rg.GUIDELINE_ID
		WHERE /*not EXISTS 
		(
...
SELECT (SELECT reviewer.DBO.TB_ITEM.ITEM_ID FROM reviewer.DBO.TB_ITEM
		...
		EXTRACT_ATTR_DESC
	FROM [epi2].EPPI_WEB.DBO.TB_EXTRACT_ATTR as src1
	INNER JOIN [epi2].EPPI_WEB.DBO.TB_ITEM_REVIEW as src ON src.ITEM_ID = src1.ITEM_ID
	inner join [epi2].EPPI_WEB.DBO.tb_ITEM i on 
	...
	AND NOT EXISTS 
		(
			...
			INNER JOIN [epi2].EPPI_WEB.DBO.TB_ITEM_REVIEW as src2 ON src2.ITEM_ID = i2.OLD_ITEM_ID COLLATE Latin1_General_CI_AS
		)
*/
...
INSERT INTO reviewer.DBO.TB_ITEM_SET(ITEM_ID, SET_ID, IS_COMPLETED, CONTACT_ID)
		...
		inner join [epi2].EPPI_WEB.DBO.tb_REVIEW_GUIDELINE rg on 
		src.GUIDELINE_ID = rg.GUIDELINE_ID
		inner join [epi2].EPPI_WEB.DBO.tb_ITEM_REVIEW ir  on ir.REVIEW_ID = rg.REVIEW_ID
...
SELECT ti.ITEM_ID ITEM_ID,
	...
	FROM [epi2].EPPI_WEB.DBO.TB_EXTRACT_ATTR as src1
	INNER JOIN [epi2].EPPI_WEB.DBO.TB_ITEM_REVIEW as src ON src.ITEM_ID = src1.ITEM_ID
	inner join [epi2].EPPI_WEB.DBO.tb_ITEM i on i.ITEM_ID = src.ITEM_ID
...
SELECT (SELECT reviewer.DBO.TB_REVIEW.REVIEW_ID FROM reviewer.DBO.TB_REVIEW
		...
		(SELECT reviewer.DBO.TB_SET.SET_ID FROM reviewer.DBO.TB_SET
				WHERE reviewer.DBO.TB_SET.OLD_GUIDELINE_ID = src.GUIDELINE_ID COLLATE Latin1_General_CI_AS),
		'FALSE'
		FROM [epi2].EPPI_WEB.DBO.TB_REVIEW_GUIDELINE as src
		WHERE src.REVIEW_ID = @revID


-- TRANSFER INDUCTIVE CODES
INSERT INTO reviewer.DBO.TB_SET(OLD_GUIDELINE_ID, SET_TYPE_ID, SET_NAME)
SELECT DISTINCT(REVIEW_ID), 3, 'Inductive codes'  FROM [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE src
WHERE src.REVIEW_ID = @revID
...
FROM [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE as src
WHERE src.REVIEW_ID = @revID
...
FROM [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE_EXTRACT_ATTR as src
INNER JOIN reviewer.DBO.TB_ITEM ON
	reviewer.DBO.TB_ITEM.OLD_ITEM_ID = src.ITEM_ID COLLATE Latin1_General_CI_AS
INNER JOIN [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE as src1 ON
...
INNER JOIN reviewer.DBO.TB_ITEM_SET ON
	...
	INNER JOIN reviewer.DBO.TB_SET ON reviewer.DBO.TB_SET.SET_ID = reviewer.DBO.TB_ITEM_SET.SET_ID
		AND reviewer.DBO.TB_SET.OLD_GUIDELINE_ID = 
					(SELECT REVIEW_ID FROM [epi2].EPPI_WEB.DBO.tb_INDUCTIVE_CODE as src1
...
SELECT DISTINCT (SELECT reviewer.DBO.TB_ITEM.ITEM_ID FROM reviewer.DBO.TB_ITEM
		...
		[IDENTITY]
FROM [epi2].EPPI_WEB.DBO.TB_EXTRACT_ATTR as src
INNER JOIN [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE_EXTRACT_ATTR as src1 ON
	src1.ATTRIBUTE_ID = src.ATTRIBUTE_ID
	AND src1.GUIDELINE_ID = src.GUIDELINE_ID
	AND src1.ITEM_ID = src.ITEM_ID
INNER JOIN [epi2].EPPI_WEB.DBO.TB_INDUCTIVE_CODE as src2 ON
...
SELECT ITEM_DOCUMENT_ID, ITEM_ATTRIBUTE_ID, START_AT, END_AT
	...
	AND src1.GUIDELINE_ID = src.GUIDELINE_ID
	AND src1.ITEM_ID = src.ITEM_ID
	inner join [epi2].EPPI_WEB.DBO.tb_INDUCTIVE_CODE src2
...
FROM [epi2].EPPI_WEB.DBO.tb_REFERENCE_ELEMENT_GROUPS src WHERE TYPE = 'EXCLUSION'
	AND src.REVIEW_ID = @revID

INSERT INTO reviewer.DBO.TB_REVIEW_SET(REVIEW_ID, SET_ID, ALLOW_CODING_EDITS, CODING_IS_FINAL)
SELECT TB_REVIEW.REVIEW_ID, SET_ID, 'True', 'True' FROM TB_SET
INNER JOIN [epi2].eppi_web.dbo.tb_REFERENCE_ELEMENT_GROUPS as src ON
...
FROM [epi2].EPPI_WEB.DBO.tb_REFERENCE_ELEMENTS as src
INNER JOIN [epi2].EPPI_WEB.DBO.tb_REFERENCE_ELEMENT_GROUPS as src1 on src.ELEMENT_GROUP_ID = src1.ELEMENT_GROUP_ID
WHERE src1.REVIEW_ID = @revID
...
INNER JOIN [epi2].EPPI_WEB.DBO.tb_REFERENCE_ELEMENT_GROUPS as src1
...
select 
	...
	0
from [epi2].eppi_web.dbo.tb_REFERENCE_ITEM_FOR_SCREENING as src
...
FROM [epi2].EPPI_WEB.DBO.tb_REFERENCE_ITEM_ELEMENTS as src
INNER JOIN reviewer.DBO.TB_ITEM ON
	reviewer.DBO.TB_ITEM.OLD_ITEM_ID = 'SCR' + CAST(src.ITEM_ID AS NVARCHAR(10)) COLLATE Latin1_General_CI_AS
INNER JOIN [epi2].EPPI_WEB.DBO.tb_REFERENCE_ELEMENTS as src1 ON
...
INNER JOIN [epi2].eppi_web.dbo.tb_REFERENCE_ELEMENT_GROUPS as src2 ON
