...
{
    [Serializable]
    public class SearchIDsCommand : CommandBase<SearchIDsCommand>
    {
#if SILVERLIGHT
    public SearchIDsCommand(){}
#else
        protected SearchIDsCommand() { }
#endif
        ...
        public SearchIDsCommand(string IDs, bool Included)
        {
            _title = "Search internal IDs: ";
            _IDs = "";
            ...
            {
                if (Int64.TryParse(s, System.Globalization.NumberStyles.Number, Ci, out ii))
                {
                    valid = true;
                    _IDs += ii.ToString("G", Ci) + ",";
                    if (_title.Length < 40)
                    {
                        _title += ii.ToString("G", Ci) + ",";
                    }
                    else if (!done)
                    {
                        _title = _title.Trim(',') + " [...]";
        ...
        protected override void DataPortal_Execute()
        {
            using (SqlConnection connection = new SqlConnection(DataConnection.ConnectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand("st_SearchItems", connection))
                {
