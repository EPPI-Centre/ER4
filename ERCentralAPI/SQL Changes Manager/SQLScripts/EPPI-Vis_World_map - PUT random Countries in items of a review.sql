...
declare @reviewID int = 501
declare @MAXno_of_countries_to_assign int = 90 --countries can be assigned more than once, though

--WARNING!!! WIPES the COUNTRY field for all included items in this review
update i set COUNTRY = '' from tb_item i inner join TB_ITEM_REVIEW ir on i.ITEM_ID = ir.ITEM_ID and ir.REVIEW_ID = @reviewID and ir.IS_DELETED = 0  and ir.IS_INCLUDED = 1

declare @countries table (countryCode varchar(2), [lat] float, [lng] float, country nvarchar(100))
insert into @countries (countryCode, [lat], [lng], country)
values 
...
declare @itemsTmp table (tmp_item_id bigint)

declare @NoOfCountries int = (select CAST(CEILING(RAND()*@MAXno_of_countries_to_assign) as INT))
declare @countryToSet nvarchar(100) = ''
declare @numberOfItems int = 100
declare @TotnumberOfItems int = (select count(*) from TB_ITEM_REVIEW where REVIEW_ID = @reviewID and IS_INCLUDED = 1 and IS_DELETED = 0)
declare @AverageNoItemsPerCountryTimes2 int = (@TotnumberOfItems/@NoOfCountries)*2
select @TotnumberOfItems as [Tot ITEMS], @AverageNoItemsPerCountryTimes2 / 2 AS AVG_ITEMS, @NoOfCountries AS NoOfCs
declare @counter int = 0
while @counter < @NoOfCountries
BEGIN
	delete from @itemsTmp
	set @counter = @counter+1
	set @numberOfItems = (select CAST(CEILING(RAND()*@AverageNoItemsPerCountryTimes2) as INT))
	set @countryToSet = (select top 1 country from @countries ORDER BY NEWID())
	select @counter, @numberOfItems, @countryToSet
	insert into @itemsTmp
	select top (@numberOfItems) i.ITEM_ID from TB_ITEM i
	...
	select i.ITEM_ID, SHORT_TITLE, COUNTRY from @itemsTmp t inner join tb_item i on i.ITEM_ID = t.tmp_item_id
END
