...
CREATE OR ALTER     procedure [dbo].[st_TransferCreditFromGhostAccount]
(
...
SET NOCOUNT ON
	...
	declare @contactIdAsString nvarchar(max)
	set @contactIdAsString = CONVERT(NVARCHAR(max), @DESTINATION_CONTACT_ID)
	...
	values (GETDATE(), 0, @DESTINATION_CONTACT_ID, @existingExpiryDate, @newExpiryDate, @EXTENDED_BY_ID, 22, 'Transfer of credit from ghost account ' + @contactIdAsString)
