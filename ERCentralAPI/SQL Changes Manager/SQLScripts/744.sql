...
/****** Object:  StoredProcedure [dbo].[st_MagReviewMagInfo]    Script Date: 08/01/2026 09:59:30 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	*/
	...
	select @NNotMatchedIncluded = count(distinct ir.ITEM_ID) from TB_ITEM_REVIEW ir
		left outer join tb_ITEM_MAG_MATCH imm on imm.ITEM_ID = ir.ITEM_ID and imm.REVIEW_ID = ir.REVIEW_ID
			where ir.IS_INCLUDED = 'true' and ir.REVIEW_ID = @REVIEW_ID and ir.IS_DELETED = 'false'
			and (imm.ITEM_ID is null or imm.ManualFalseMatch = 'true')

	select @NNotMatchedExcluded = count(distinct ir.ITEM_ID) from TB_ITEM_REVIEW ir
		left outer join tb_ITEM_MAG_MATCH imm on imm.ITEM_ID = ir.ITEM_ID and imm.REVIEW_ID = ir.REVIEW_ID
			where ir.IS_INCLUDED = 'false' and ir.REVIEW_ID = @REVIEW_ID and ir.IS_DELETED = 'false'
			and (imm.ITEM_ID is null or imm.ManualFalseMatch = 'true')
	--
...
/****** Object:  StoredProcedure [dbo].[st_ItemListMagNoMatches]    Script Date: 08/01/2026 10:00:17 ******/
SET ANSI_NULLS ON
GO
...
Declare @ID table (ItemID bigint primary key )
      ...
      INNER JOIN TB_ITEM_REVIEW ON TB_ITEM_REVIEW.ITEM_ID = I.ITEM_ID AND 
			...
			-- new
			where (imm.ITEM_ID is null or imm.ManualFalseMatch = 'true')
