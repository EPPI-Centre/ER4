...
SET NOCOUNT ON
	...
	UPDATE A
		...
		JOIN
		TB_TRAINING_FROM_SEARCH_ITEM B ON A.ITEM_ID = B.ITEM_ID AND CURRENT_TIMESTAMP < DATEADD(DAY, 7, B.WHEN_LOCKED) AND
...
-- NEXT, TRY TO LOCK THE ITEM WE'RE GOING TO SEND BACK (BUT WE WON'T OVERRIDE SOMEONE ELSE'S LOCK)
	...
	ELSE
	BEGIN
		declare @trainingFSId int = (select top 1 TRAINING_FS_ITEM_ID from TB_TRAINING_FROM_SEARCH_ITEM 
