USE [Reviewer]
GO
/****** Object:  StoredProcedure [dbo].[st_WebDbCountryFrequency]    Script Date: 26/01/2024 09:54:58 ******/
SET ANSI_NULLS OFF
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE or ALTER     PROCEDURE [dbo].[st_WebDbCountryFrequency]
(	
		@RevId int
		, @WebDbId int
		, @Included bit = null
...
declare @itemsTmp table (countryCode varchar(2), [lat] float, [lng] float, country nvarchar(100),  [value] int)
insert into @itemsTmp (countryCode, [lat], [lng], country)
values 
...
declare @items table (ItemId bigint, countryField nvarchar(50), primary key (ItemId, countryField))
	...
	Begin 
		insert into @items 
		select count(distinct i.item_id) AS CountryCount, COUNTRY from TB_ITEM i
	...
	set [value] = i.ItemId
	from @items i
	where country = countryField 

	select * from @itemsTmp
	where [value] != 0
...
USE [Reviewer]
GO
/****** Object:  StoredProcedure [dbo].[st_WebDbSearchFreeText]    Script Date: 26/01/2024 09:56:09 ******/
SET ANSI_NULLS OFF
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER   PROCEDURE [dbo].[st_WebDbSearchFreeText]
(
     @RevId int 
    ,@WebDbId int
      ...
      BEGIN
		...
		if @len = 4
			...
			INNER JOIN TB_ITEM ON TB_ITEM.ITEM_ID = i.ItemId
				...
				OR TB_ITEM.[YEAR] ='   ' + @SEARCH_TEXT
      END
	ELSE IF (@SEARCH_WHAT = 'Country')   ---- this is the new part that was added
      BEGIN
            UPDATE @items set found = 1 from @items i
			INNER JOIN TB_ITEM ON TB_ITEM.ITEM_ID = i.ItemId
            WHERE TB_ITEM.[COUNTRY] =  @SEARCH_TEXT
		END
...
-- useful script for assigning countrys to items (for testing purposes)
/*
declare @countryToSet nvarchar(100) = 'Brazil'
declare @reviewID int = 568
declare @numberOfItems int = 100

declare @itemsTmp table (tmp_item_id bigint, country nvarchar(100))

insert into @itemsTmp
select top @numberOfItems i.ITEM_ID, i.COUNTRY from TB_ITEM i
inner join TB_ITEM_REVIEW ir on ir.ITEM_ID = i.ITEM_ID
where ir.REVIEW_ID = @reviewID
and i.COUNTRY = '' and i.ABSTRACT != ''
...
from @itemsTmp it
where it.tmp_item_id = ITEM_ID

select i.ITEM_ID, i.COUNTRY, i.ABSTRACT from TB_ITEM i
inner join TB_ITEM_REVIEW ir on ir.ITEM_ID = i.ITEM_ID
where ir.REVIEW_ID = @reviewID
