...
export class ItemListComp implements OnInit, OnDestroy {
  ...
  OpenItem(itemId: number) {
    ...
    if (itemId > 0) {
      ...
      if (this.Context == 'FullUI' || this.Context == 'dedup') this.router.navigate(['itemcoding', itemId]);
  ...
  private Clear() {
    console.log("Clear in ItemListComponent");
    this.ShowOptions = false;
    this.ShowSelectionHelp = false;
    this._LocalPageSize = null;
  }
  ngOnDestroy() {
    console.log("destroy ItemList component.");
