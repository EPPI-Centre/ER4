...
export class ComparisonReconciliationComp extends BusyAwareService implements OnInit, OnDestroy {
  ...
  public IsCodingOnly: boolean = false;

  ngOnInit() {
    console.log("Reconcile page url:", this.router.url);
    if (this.router.url.endsWith("ReconciliationCO")) {
      this.IsCodingOnly = true;
    }
  ...
  OpenItem(itemId: number) {
    if (itemId > 0) {
      if (this.IsCodingOnly) this.router.navigate(['itemcodingOnly', itemId]);
      else this.router.navigate(['itemcoding', itemId]);
    }
  }
  FirstItem() {
  ...
  BackToMain() {
    if (this.IsCodingOnly) this.router.navigate(['MainCodingOnly']);
