...
export class ComparisonReconciliationComp extends BusyAwareService implements OnInit, OnDestroy {
  ...
  private PrepareDetailsViewData() {
    if (this.panelItem) {
      ...
      this.SetMatchOutcomes();
    }
  }
  
  private SetMatchOutcomes() {
    if (this.allItems[this.selectedRow] && (
      this.allItems[this.selectedRow].OutcomesReviewer1.length > 0
      || this.allItems[this.selectedRow].OutcomesReviewer2.length > 0 || this.allItems[this.selectedRow].OutcomesReviewer3.length > 0
    )) {
      let MatchedOutcomeIds = this.ItemCodingService.MatchOutcomes(this.allItems[this.selectedRow].OutcomesReviewer1
        , this.allItems[this.selectedRow].OutcomesReviewer2
        , this.allItems[this.selectedRow].OutcomesReviewer3);

      this.allItems[this.selectedRow].SetMatchedOutcomes(MatchedOutcomeIds);

      this.allItems[this.selectedRow].UnMatchedOutcomesReviewer1 = this.ItemCodingService.GetUnmatchedOutcomes(
        this.allItems[this.selectedRow].OutcomesReviewer1, MatchedOutcomeIds, 0);

      this.allItems[this.selectedRow].UnMatchedOutcomesReviewer2 = this.ItemCodingService.GetUnmatchedOutcomes(
        this.allItems[this.selectedRow].OutcomesReviewer2, MatchedOutcomeIds, 1);

      if (this.CurrentComparison.contactName3 != '') {
        this.allItems[this.selectedRow].UnMatchedOutcomesReviewer3 = this.ItemCodingService.GetUnmatchedOutcomes(
          this.allItems[this.selectedRow].OutcomesReviewer3, MatchedOutcomeIds, 2);
      }
      this.SetMatchedOutcomesHTML();
    }
  }
  private SetMatchedOutcomesHTML() {
    let res = "";
    ...
    res = "";
    if (this.allItems[this.selectedRow].UnMatchedOutcomesReviewer1.length > 0) {
      res += '<div class="border-bottom border-info col-12">Unmatched Outcomes for <strong>'
        + this.CurrentComparison.contactName1 + "</strong>"
        + this.OutcomeHTMLtable(this.allItems[this.selectedRow].UnMatchedOutcomesReviewer1) + "</div>";
    }
    if (this.allItems[this.selectedRow].UnMatchedOutcomesReviewer2.length > 0) {
      res += '<div class="border-bottom border-info col-12">Unmatched Outcomes for <strong>'
        + this.CurrentComparison.contactName2 + "</strong>"
        + this.OutcomeHTMLtable(this.allItems[this.selectedRow].UnMatchedOutcomesReviewer2) + "</div>";
    }
    if (this.allItems[this.selectedRow].UnMatchedOutcomesReviewer3.length > 0) {
      res += '<div class="border-bottom border-info col-12">Unmatched Outcomes for <strong>'
        + this.CurrentComparison.contactName3 + "</strong>"
        + this.OutcomeHTMLtable(this.allItems[this.selectedRow].UnMatchedOutcomesReviewer3) + "</div>";
    }
