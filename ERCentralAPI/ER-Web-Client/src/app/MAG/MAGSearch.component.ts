...
export class MAGSearchComponent implements OnInit {
  ...
  public isCopiedFromOpenAlex: boolean = false;
  public showFullTextForNameOfThisSearchId: number = -1;
  public showFullTextForSearchStringOfThisSearchId: number = -1;
  public readonly truncateSearchTextAtIndex: number = 54;
  ...
  public TruncateThis(value: string, completeWords = false, ellipsis = '...') {
    let limit = this.truncateSearchTextAtIndex;
    if (completeWords) {
  ...
  public DisplayStyleForName(search: MagSearch): number {
    if (search.searchText.length <= this.truncateSearchTextAtIndex) return 0;
    if (search.magSearchId == this.showFullTextForNameOfThisSearchId) return 1;
    return 2;
  }
  public DisplayStyleForSearchString(search: MagSearch): number {
    if (search.magSearchText.length <= this.truncateSearchTextAtIndex) return 0;
    if (search.magSearchId == this.showFullTextForSearchStringOfThisSearchId) return 1;
    return 2;
  }
  ...
  public RunOpenAlexSearch() {
    ...
    switch (this.WordsInSelection) {
      ...
      case "3":
        ...
        break;
      case "4":
        let Qstring = this.magSearchInput;
        if (this.isCopiedFromOpenAlex) {
          const splitOnQueryString = Qstring.split('?');
          if (splitOnQueryString.length != 2) {
            this.modalService.GenericErrorMessage("Sorry, could not parse the API Query. <br />Could not isolate the 'Query string' part of the URL.");
            this.magSearchInput = "";
            this.isCopiedFromOpenAlex = false;
            return;
          }
          const splitOnQueryVariables = splitOnQueryString[1].split('&');
          const index = splitOnQueryVariables.findIndex(f => f.startsWith("filter="));
          if (index == -1) {
            this.modalService.GenericErrorMessage("Sorry, could not parse the API Query. <br />Could not isolate the 'filter' part of the URL.");
            this.magSearchInput = "";
            this.isCopiedFromOpenAlex = false;
            return;
          }
          else {
            Qstring = splitOnQueryVariables[index];
            Qstring = Qstring.substring("filter=".length);
            this.magSearchInput = decodeURIComponent(Qstring);
            this.isCopiedFromOpenAlex = false;
