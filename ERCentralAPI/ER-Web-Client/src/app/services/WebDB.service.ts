...
export class WebDBService extends BusyAwareService implements OnDestroy {
  ...
  public CloneWebDBforEdit(toClone: iWebDB): iWebDB {
    let res = {
      ...
      SplittedHiddenFields: toClone.hiddenFields.split(",")
    }
    res.SplittedHiddenFields = res.SplittedHiddenFields.filter(f => f !== '');
    return res;
  }
...
export interface iWebDB {
  ...
  hiddenFields: string;
  SplittedHiddenFields: string[] | undefined;//populated only by CloneWebDBforEdit(iwebdb) method, to ensure we do the "split" work only once, when needed
}
