...
export class ComparisonsService extends BusyAwareService implements OnDestroy {
  ...
  public CreateAllComparisons(setId: number): Promise<number | false> {

    this._BusyMethods.push("CreateAllComparisons");
    //console.log('inside the service now' + JSON.stringify(comparison));
    return lastValueFrom(this._httpC.post<number>(this._baseUrl +
      'api/Comparisons/CreateAllComparisonsForThisSet', setId))
      .then(
        cmdrestNumber => {
          this.RemoveBusy("CreateAllComparisons");
          if (cmdrestNumber > 0) this.FetchAll();
          return cmdrestNumber;
        }
        , error => {
          this.RemoveBusy("CreateAllComparisons");
          this.modalService.GenericError(error);
