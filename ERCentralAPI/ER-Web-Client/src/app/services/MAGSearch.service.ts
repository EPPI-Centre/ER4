...
export class magSearchService extends BusyAwareService {
  ...
  FetchMAGSearchList() {
    ...
    return lastValueFrom(this._httpC.get<MagSearch[]>(this._baseUrl + 'api/MAGSearchList/FetchMagSearchList'))
      .then(result => {
  ...
  ImportMagSearches(magSearchId: string, searchText: string,
    ...
    return lastValueFrom(this._httpC.post<number>(this._baseUrl + 'api/MAGSearchList/ImportMagSearchPapers',
      body))
  ...
  EnumerateSearchItems(search: MagSearch) {
    this._BusyMethods.push("EnumerateSearchItems");
    let body = JSON.stringify({
      Value: search.magSearchId
    });
    return lastValueFrom(this._httpC.post<MagSearch[]>(this._baseUrl + 'api/MAGSearchList/EnumerateSearchItems',
      body))

      .then(result => {
        this.RemoveBusy("EnumerateSearchItems");
        //console.log(result);
        this.MagSearchList = result;
        for (var i = 0; i < result.length; i++) {
          result[i].add = false;
